...
use syn_engine::grace::contract_generator::{ContractGenerator, ContractRepairRequest};
...
pub(crate) async fn handle_diagnose_failure(
    ...
    match syn_engine::grace::failure_diagnosis::diagnose_failure_text(&root, report) {
        Ok(report) => result(
            id,
            serde_json::json!({
