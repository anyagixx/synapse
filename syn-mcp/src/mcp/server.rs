...
use syn_engine::graphrag::GraphRag;
use syn_engine::indexer::storage::Storage;
use syn_engine::indexer::Indexer;
use syn_skills::skills::{SkillEngine, SkillRequest};
use syn_core::tracking::{mcp_metrics::McpCallStatus, BudgetLevel, BudgetStatus, Tracker};
...
impl McpServer {
    ...
    pub async fn start_stdio(self) -> anyhow::Result<()> {
        ...
        let pipeline_handler: PipelineHandler = Arc::new(move |line| {
            ...
            Box::pin(async move {
                tracing::debug!("MCP << {}", syn_core::utils::truncate_chars(&line, 200));
                let response = handler.handle_message(&line).await;
                if let Some(response) = &response {
                    if let Ok(msg) = serde_json::to_string(response) {
                        tracing::debug!("MCP >> {}", syn_core::utils::truncate_chars(&msg, 200));
                    }
                }
                response
            })
...
fn preload_index_storage(indexer: &Indexer, root: &Path) -> anyhow::Result<()> {
    ...
    if guard.is_none() {
        *guard = Some(syn_engine::indexer::storage::Storage::new(root));
    }
    Ok(())
}
...
mod tests {
    ...
    // ── Stress tests (Phase-103) ──

    #[tokio::test]
    async fn stress_concurrent_tool_calls_do_not_deadlock() {
        let data_home = tempfile::tempdir().expect("data home");
        let project_root = tempfile::tempdir().expect("project root");
        ...
        let tracker = Tracker::new_for_test(
            ...
            data_home.path(),
            project_root.path(),
            Some("stress-concurrent"),
        );
        let handler = std::sync::Arc::new(SynapseHandler::new_for_test(config, tracker));
        handler
            .handle_message(r#"{"jsonrpc":"2.0","id":0,"method":"initialize"}"#)
            .await
            .expect("init");
        ...
        for i in 1..=10 {
            ...
            handles.push(tokio::spawn(async move {
                ...
                h.handle_message(&request).await
            }));
        }
        for handle in handles {
            let result = handle.await.expect("join").expect("handle");
            assert!(result["result"]["tools"].is_array());
        }
    }

    #[tokio::test]
    async fn stress_large_payload_is_rejected_by_budget_gate() {
        let data_home = tempfile::tempdir().expect("data home");
        let project_root = tempfile::tempdir().expect("project root");
        let mut config = Config::default();
        config.budget.session_token_limit = 10;
        config.budget.block_at_pct = 100;
        let tracker = Tracker::new_for_test(
            &config,
            data_home.path(),
        ...
        // Simulate near-limit spending
        tracker.record("spent", 9, 200).await.expect("seed spend");
        let handler = SynapseHandler::new_for_test(config, tracker);
        handler
            .handle_message(r#"{"jsonrpc":"2.0","id":0,"method":"initialize"}"#)
            .await
        ...
        // Any tool call should be blocked at this budget level
        let response = handler
            .handle_message(
                r#"{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"semantic_search","arguments":{"query":"test","max_results":5}}}"#,
            )
            .await
            .expect("response");
        // Budget gate should either block or warn
        let has_budget = response["result"]["_meta"]["budget"].is_object()
            || response["error"].is_object();
        assert!(has_budget);
    }

    #[tokio::test]
    async fn stress_timeout_cascade_recovers_after_slow_tool() {
        let data_home = tempfile::tempdir().expect("data home");
        let project_root = tempfile::tempdir().expect("project root");
        let config = Config::default();
        let tracker = Tracker::new_for_test(
        ...
        // Send a fast tools/list request after a potentially slow one — system must stay responsive
        let response = handler
            .handle_message(r#"{"jsonrpc":"2.0","id":1,"method":"tools/list"}"#)
            .await
            .expect("tools/list");
        assert!(response["result"]["tools"].is_array());
        assert!(response["result"]["tools"].as_array().is_some_and(|t| t.len() >= 40));
    }
}
// END_public_api
