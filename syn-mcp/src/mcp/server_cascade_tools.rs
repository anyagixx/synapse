...
use syn_engine::grace::cascade::{CascadeExecuteOptions, CascadeReport};
use std::path::Path;
...
async fn handle_cascade_impact_at(
    ...
    match syn_engine::grace::GraceEngine::cascade_impact(root, changed_artifact, change_description) {
        Ok(analysis) => {
            let mut text = analysis.preview.clone();
            let mut execution: Option<CascadeReport> = None;
            if !preview_only {
                match syn_engine::grace::GraceEngine::cascade_execute(
                    root,
...
async fn handle_cascade_execute_at(
    ...
    match syn_engine::grace::GraceEngine::cascade_execute(root, options) {
        Ok(report) => {
