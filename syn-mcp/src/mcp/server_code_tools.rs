...
use syn_engine::grace::contract::LinkType;
use syn_engine::graphrag::{render_mermaid, GraphRag, MermaidGraphSubset, MermaidRenderOptions};
use syn_engine::indexer::storage::SearchFilters;
...
fn terse_text_preview(text: &str) -> String {
    ...
    if lines.len() <= TERSE_LINES {
        return lines
            .iter()
            .map(|line| syn_core::utils::truncate_chars(line, TERSE_LINE_CHARS))
            .collect::<Vec<_>>()
            .join("\n");
    }
            ...
            let text = if results.is_empty() {
                ...
                results
                    ...
                    .map(|(i, r)| {
                        ...
                        let preview = syn_core::utils::truncate_chars(&r.content, 150);
                        format!(
                            "{}. {} ({}:{}-{})\n   {}\n   reason: {}",
                            i + 1,
                            r.path,
...
fn format_typed_relationship(rel: &syn_engine::graphrag::types::TypedCodeRelationship) -> String {
