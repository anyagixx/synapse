...
pub(crate) async fn handle_verify(
    ...
    match syn_engine::grace::GraceEngine::verify_project_with_profile(&root, profile).await {
        Ok(results) => {
...
pub(crate) async fn handle_review(
    ...
    match syn_engine::grace::review::Reviewer::review_with_profile(&root, mode, profile) {
        Ok(report) => {
            let mut text = format!(
...
pub(crate) async fn handle_status(
    ...
    match syn_engine::grace::status::StatusCollector::collect(&root).await {
        Ok(report) => {
...
pub(crate) async fn handle_analyze_logs(
    ...
    match syn_engine::grace::log::analyze_log_file(&path, mode, contract_ref) {
...
pub(crate) async fn handle_extract_belief_state(
    ...
    match syn_engine::grace::belief_state::extract_and_store_belief_state(&root, module_id, context) {
        Ok(report) => {
            let text = format!(
...
pub(crate) async fn handle_generate_requirements(
    ...
    match syn_engine::grace::requirements::generate_requirements_file(
        &root,
        project_description,
        domain,
        detail_level,
...
pub(crate) async fn handle_generate_technology(
    ...
    match syn_engine::grace::technology::generate_technology_file(
        &root,
        detect_existing,
        compatibility_check,
    ) {
...
pub(crate) async fn handle_generate_development_plan(
    ...
    match syn_engine::grace::development_plan::generate_development_plan_file(
        &root,
        from_requirements,
        data_flow_analysis,
        generation_order,
...
pub(crate) async fn handle_mental_test_run(
    ...
    match syn_engine::grace::mental_test::run_mental_test(&root, module_id, mental_test_id, step_by_step)
    {
        Ok(report) => text_result(id, report.to_xml(), args),
...
pub(crate) async fn handle_traceability_report(
    ...
    match syn_engine::grace::traceability::scan_project_traceability(&root) {
        Ok(report) => {
            let index_path = syn_engine::grace::traceability::write_traceability_index(&root, &report)
                .map(|path| path.display().to_string())
                .unwrap_or_else(|_| "docs/traceability-index.xml not written".into());
            let mut text = syn_engine::grace::traceability::format_traceability_report(
...
pub(crate) async fn handle_run_test_guide(
    ...
    match syn_engine::grace::testing::run_test_guide(
        &root,
        guide_path,
        application_url,
        agent_console_url,
...
pub(crate) async fn handle_submit_test_report(
    ...
    match syn_engine::grace::testing::submit_test_report(&root, report_path, to) {
        Ok(report) => {
            let text = serde_json::to_string_pretty(&report).unwrap_or_default();
            result(
                id,
...
pub(crate) async fn handle_self_heal(
    ...
    let manager = syn_run::RunManager::new(root);
...
pub(crate) async fn handle_gain(
    ...
    let config = syn_core::config::Config::load().unwrap_or_default();
...
pub(crate) async fn handle_compress(
    ...
    let compressor = syn_core::compress::Compressor::new(&cfg);
...
mod tests {
    ...
    async fn test_handle_extract_belief_state_writes_artifact() {
        let dir = tempfile::tempdir().expect("tempdir");
        let layout = syn_engine::grace::layout::DocsLayout::new(dir.path());
