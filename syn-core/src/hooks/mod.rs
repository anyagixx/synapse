...
pub fn merge_synapse_mcp_config(existing: &str) -> anyhow::Result<String> {
    ...
    // Inject GRACE instruction file references for ALL modes (Plan, Build, Chat)
    // OpenCode 1.15.11+ requires instructions to be an array of file paths/globs, not inline text
    root.entry("instructions".to_string())
        .or_insert_with(|| serde_json::json!([
            "AGENTS.md",
            ".opencode/rules/grace-mandate.md",
