...
use syn_engine::grace::contract_generator::{ContractGenerator, ContractRepairResult};
use syn_engine::grace::verify::VerificationResult;
use syn_engine::grace::{GraceEngine, GraceProfile};
...
fn diagnose_sync(
    root: PathBuf,
    description: String,
) -> anyhow::Result<syn_engine::grace::fix::FixResult> {
    std::thread::spawn(move || {
        let runtime = tokio::runtime::Builder::new_current_thread()
            .enable_all()
            .build()?;
        runtime.block_on(syn_engine::grace::fix::Debugger::diagnose(&description, &root))
    })
    .join()
