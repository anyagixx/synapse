...
mod tests {
    ...
    // START_CONTRACT_handoff_role_chain_is_acyclic
    // PURPOSE: Verify the GRACE role chain (Planner→Implementer→Reviewer→Verifier→Fixer→Releaser) is correct.
    // START_handoff_role_chain_is_acyclic
    #[test]
    fn handoff_role_chain_is_acyclic() {
        assert_eq!(HandoffRole::Planner.next(), Some(HandoffRole::Implementer));
        assert_eq!(HandoffRole::Implementer.next(), Some(HandoffRole::Verifier));
        assert_eq!(HandoffRole::Verifier.next(), Some(HandoffRole::Fixer));
        assert_eq!(HandoffRole::Fixer.next(), Some(HandoffRole::Reviewer));
        assert_eq!(HandoffRole::Reviewer.next(), Some(HandoffRole::Releaser));
        assert_eq!(HandoffRole::Releaser.next(), None);
    }
    // END_handoff_role_chain_is_acyclic

    // START_CONTRACT_latest_handoff_returns_none_for_empty_handoffs
    // PURPOSE: Verify latest_handoff returns None when no handoffs exist for a run.
    // START_latest_handoff_returns_none_for_empty_handoffs
    #[test]
    fn latest_handoff_returns_none_for_empty_handoffs() {
        let root = tempfile::tempdir().unwrap();
        let manager = RunManager::new(root.path());
        let run_id = saved_run(&manager);

        let result = manager.latest_handoff(&run_id).unwrap();
        assert!(result.is_none());
    }
    // END_latest_handoff_returns_none_for_empty_handoffs
}
