...
impl RunManager {
    ...
    // INPUTS: { request: RunScenarioRequest }, { report: &syn_engine::grace::status::StatusReport }
    ...
    pub fn run_scenario_from_report(
        ...
        report: &syn_engine::grace::status::StatusReport,
...
mod tests {
    ...
    #[test]
    fn blocked_review_scenario_produces_review_approval_events() {
        let root = tempfile::tempdir().unwrap();
        let manager = RunManager::new(root.path());
        ...
        assert_eq!(result.run.status, RunStatus::Completed);
        assert!(result.replay.events.iter().any(|event| event.kind == "review"));
        assert!(result.evidence_refs.len() >= 4);
    }

    #[test]
    fn scenario_request_serializes_phases_correctly() {
        let req = request(RunScenarioMode::HappyPath);
        let json = serde_json::to_string(&req).unwrap();
        let deserialized: RunScenarioRequest = serde_json::from_str(&json).unwrap();
        assert_eq!(deserialized.phase, "Phase-22");
        assert_eq!(deserialized.module_id, "M-RUNNER");
        assert_eq!(deserialized.mode, RunScenarioMode::HappyPath);
    }

    #[test]
    fn scenario_run_id_is_unique_per_execution() {
        let root = tempfile::tempdir().unwrap();
        let manager = RunManager::new(root.path());
        ...
        assert_ne!(result1.run.run_id, result2.run.run_id);
    }
}
