...
impl Proxy {
    ...
    pub async fn execute(&self, cmd_parts: &[String]) -> anyhow::Result<String> {
        ...
        let input_tokens = crate::utils::estimate_tokens(&raw_output) as u32;
        ...
        let output = match self.engine.find_filter(&full_cmd) {
            ...
            None => {
                ...
                if raw_output.len() > max_chars {
                    format!("{}...\n[output truncated at {} chars]", &raw_output[..max_chars], max_chars)
                } else {
                    raw_output
                }
            }
        ...
        // Track savings
        self.tracker.record(&full_cmd, input_tokens, output_tokens).await.ok();

        tracing::debug!(
            "Proxy: {} — {} tokens → {} tokens ({}% saved)",
            full_cmd, input_tokens, output_tokens,
