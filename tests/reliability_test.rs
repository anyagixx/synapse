...
// run_syn — Runs syn in an isolated project and data home
...
// LAST_CHANGE: [v1.0.0 — Added corrupted index reliability regression]
// END_CHANGE_SUMMARY
...
// START_syn_bin
fn syn_bin() -> PathBuf {
    std::env::current_dir().unwrap().join("target/debug/syn")
}
// END_syn_bin

// START_CONTRACT_run_syn
// PURPOSE: Run syn with an isolated project root and XDG data home
...
fn corrupt_first_blocks_json(data_home: &Path) {
    let index_root = data_home.join("synapse").join("index");
    let blocks_path = std::fs::read_dir(index_root)
        .unwrap()
        .filter_map(|entry| entry.ok().map(|e| e.path().join("blocks.json")))
        .find(|path| path.exists())
        .expect("blocks.json should exist after indexing");
