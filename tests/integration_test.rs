...
// SCOPE: init, clean config bootstrap, tracking identity, index, search, verify, status, run scenario/action queue, proxy, RTK shortcuts, local RTK adapters, rewrite hook decisions including safe shell chains, pipelines, and fd redirects, route preview, raw evidence, filter trust/verification, gain, doctor, hooks, compress
...
// test_rewrite_cli_delegates_to_router — Verifies hook-facing command rewrites, safe chains, pipelines, and fd redirects use proxy router decisions
...
// LAST_CHANGE: [v4.7.0 — Added fd-redirect RTK rewrite integration coverage]
// END_CHANGE_SUMMARY
...
// PURPOSE: Verify syn rewrite prints routeable proxy commands, rewrites safe shell chains/pipelines/fd redirects, and skips unsupported commands for thin hooks
...
fn test_rewrite_cli_delegates_to_router() {
    ...
    assert!(
        out.status.success(),
        "syn rewrite compound command failed: {}",
        String::from_utf8_lossy(&out.stderr)
    );
    assert_eq!(
        String::from_utf8_lossy(&out.stdout).trim(),
    ...
    assert!(
        out.status.success(),
        "syn rewrite fd redirect command failed: {}",
        String::from_utf8_lossy(&out.stderr)
    );
    assert_eq!(
        String::from_utf8_lossy(&out.stdout).trim(),
