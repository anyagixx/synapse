...
// PURPOSE: Verify syn rewrite prints routeable proxy commands, rewrites safe shell chains/pipelines/fd redirects/shell prefixes, and skips unsupported commands for thin hooks
...
fn test_rewrite_cli_delegates_to_router() {
    ...
    let out = Command::new(&syn)
        .args(["rewrite", "sudo nocorrect git status && builtin cargo test"])
        .output()
        .unwrap();
    assert!(
        out.status.success(),
        "syn rewrite shell prefix command failed: {}",
        String::from_utf8_lossy(&out.stderr)
    );
    assert_eq!(
        String::from_utf8_lossy(&out.stdout).trim(),
