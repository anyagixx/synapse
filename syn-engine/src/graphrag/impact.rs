...
mod tests {
    ...
    #[test]
    fn test_analyze_impact_unknown_node_returns_none() {
        let result = analyze_impact(&sample_graph(), "M-NONEXISTENT", 2, true);
        assert!(result.is_none());
    }

    #[test]
    fn test_analyze_impact_empty_graph_returns_none() {
        let empty = CodeGraph::default();
        let result = analyze_impact(&empty, "M-ANY", 2, true);
        assert!(result.is_none());
    }
}
