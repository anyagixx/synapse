...
# SCOPE: Tag/version and release-context freshness guards, checked-out candidate SHA evidence, generated release notes policy, checksum aggregation policy, supported Linux/macOS installer dry-run mapping, full RTK release gate, GitHub step-summary evidence, and optional local release smoke.
...
# check_installer_matrix - Validates supported Linux x86_64/aarch64 and macOS arm64 installer mappings
...
# LAST_CHANGE: [v1.7.0 - Removed Intel macOS from prebuilt release matrix until ONNX Runtime artifacts exist]
# END_CHANGE_SUMMARY
...
write_step_summary() {
...

    {
        ...
        echo "- Installer matrix: Linux x86_64, Linux aarch64, macOS arm64"
