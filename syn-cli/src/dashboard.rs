...
use syn_engine::grace::belief_state::collect_project_belief_states;
use syn_engine::grace::cascade::cascade_impact;
use syn_engine::grace::cascade_change::list_changelog_entries;
use syn_engine::grace::mental_test::scan_project_mental_tests;
use syn_engine::grace::status::StatusCollector;
use syn_engine::grace::traceability::{scan_project_traceability, TraceabilityChain};
use axum::{
...
async fn index_html() -> Html<String> {
    ...
    let layout = syn_engine::grace::layout::DocsLayout::new(&root);
...
async fn api_tokens() -> Json<Value> {
    let config = syn_core::config::Config::load_or_default();
...
async fn api_search_explain_at_root(root: PathBuf, query: SearchExplainQuery) -> Json<Value> {
    ...
    let indexer = syn_engine::indexer::Indexer::new(&syn_core::config::Config::load_or_default());
...
fn belief_states_payload(root: &Path) -> Value {
    let summary = match syn_engine::grace::belief_state::scan_project_belief_states(root) {
        Ok(report) => report,
...
mod tests {
    use super::*;
    use syn_engine::grace::belief_state::{
        BeliefState, BeliefStrategy, BeliefUnderstanding, DataFlowBelief,
    };
    use syn_engine::grace::cascade_change::{new_change_log, write_changelog, CascadeChange};
    ...
    fn test_belief_states_payload_returns_state_rows() {
        let _cwd = syn_core::utils::test_cwd_lock().blocking_lock();
    ...
    async fn test_runs_payload_reports_runs_and_events() {
        ...
        let manager = syn_run::RunManager::new(dir.path());
        let run = syn_run::RunRecord::new(
            "goal".into(),
            "Phase-17".into(),
            "M-RUNNER".into(),
            "objective".into(),
        ...
        let blocked = syn_run::RunRecord::new(
            "blocked goal".into(),
            "Phase-17".into(),
            "M-RUNNER".into(),
    ...
    async fn test_health_ready_reports_mcp_state() {
        ...
        let tracker = syn_core::tracking::Tracker::new_for_test(
            &syn_core::config::Config::default(),
            data_home.path(),
            project_home.path(),
    ...
    async fn test_api_search_explain_returns_reasoned_results() {
        ...
        .expect("write source");
        let indexer = syn_engine::indexer::Indexer::new(&syn_core::config::Config::default());
