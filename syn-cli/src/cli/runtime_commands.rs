...
use syn_run::scenario::{RunScenarioMode, RunScenarioRequest, RunScenarioResult};
...
impl GainCmd {
    ...
    pub async fn run(&self, config: Config) -> anyhow::Result<()> {
        let tracker = syn_core::tracking::Tracker::new(&config);
...
fn print_savings_graph(items: &[syn_core::tracking::TrackingCommandStat]) {
    if items.is_empty() {
        return;
    }
    let max_saved = items
...
fn print_adapter_stats(items: &[syn_core::tracking::TrackingAdapterStat]) {
    if items.is_empty() {
        return;
    }
    println!();
...
fn print_session_stats(items: &[syn_core::tracking::TrackingSessionStat]) {
    if items.is_empty() {
        return;
    }
    println!();
...
impl ProxyCmd {
    ...
    pub async fn run(&self, config: Config) -> anyhow::Result<()> {
        ...
        let proxy = syn_proxy::proxy::Proxy::new(&config);
    ...
    pub async fn run(&self, _config: Config) -> anyhow::Result<()> {
        match &self.action {
            FiltersAction::Verify(cmd) => {
                let engine = syn_proxy::proxy::toml_filter::FilterEngine::new();
                if cmd.list {
            ...
            FiltersAction::Trust => {
                let entry = syn_proxy::proxy::filter_trust::trust_project_filters()?;
            ...
            FiltersAction::Untrust => {
                let removed = syn_proxy::proxy::filter_trust::untrust_project_filters()?;
                if removed {
            ...
            FiltersAction::Status => {
                let status = syn_proxy::proxy::filter_trust::project_filter_status()?;
                println!("Project filter status: {}", status.label());
                if let syn_proxy::proxy::filter_trust::TrustStatus::ContentChanged {
                    trusted_sha256,
                    current_sha256,
                } = status
                {
...
fn print_filter_names(engine: &syn_proxy::proxy::toml_filter::FilterEngine) {
...
fn print_filter_verify_results(
    results: &syn_proxy::proxy::toml_filter::FilterVerifyResults,
    require_all: bool,
) {
...
fn print_route_decision(decision: &syn_proxy::proxy::router::RouteDecision) {
...
impl CompressCmd {
    ...
    // START_compress_run
    pub async fn run(&self, config: Config) -> anyhow::Result<()> {
        let compressor = syn_core::compress::Compressor::new(&config);
...
impl McpCmd {
    ...
    // START_mcp_run
    pub async fn run(&self, config: Config) -> anyhow::Result<()> {
        let server = syn_mcp::mcp::server::McpServer::new(config);
...
impl HooksCmd {
    ...
    // START_hooks_run
    pub async fn run(&self, config: Config) -> anyhow::Result<()> {
        let manager = syn_core::hooks::HookManager::new(&config);
...
impl DoctorCmd {
    ...
    pub async fn run(&self, _config: Config) -> anyhow::Result<()> {
        ...
        let storage = syn_engine::indexer::storage::Storage::new(&root);
        ...
        let parser = syn_engine::indexer::parser::ParserEngine::new();
        ...
        let db = syn_core::tracking::Tracker::db_path().ok();
        check!(
            "sqlite tracking",
            db.is_some(),
        ...
        let has_sources = !syn_engine::indexer::walker::Walker::new(&root).walk().is_empty();
        check!(
            "sources",
            has_sources,
            "source files found",
...
fn check_project_dependencies(root: &Path, force: bool) -> anyhow::Result<u32> {
    ...
    let has_python_files = syn_engine::indexer::walker::Walker::new(root)
        .walk()
        .iter()
