...
use syn_proxy::proxy::toml_filter::{FilterDryRunReport, FilterEngine};
use anyhow::Context;
use std::io::Read;
...
fn render_filter_dry_run_json(
    ...
    let input_tokens = syn_core::utils::estimate_tokens(input);
    let output_tokens = syn_core::utils::estimate_tokens(&report.output);
