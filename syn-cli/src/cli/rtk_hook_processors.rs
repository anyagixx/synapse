    ...
    pub async fn run(&self, _config: Config) -> anyhow::Result<()> {
        match &self.command {
            HookProcessorAction::Install => {
                let manager = syn_core::hooks::HookManager::new(&_config);
...
// INPUTS: { verb: &str }, { report: &syn_core::hooks::GitHookReport }
// OUTPUTS: { anyhow::Result<()> }
...
fn print_git_hook_report(verb: &str, report: &syn_core::hooks::GitHookReport) -> anyhow::Result<()> {
    println!(
...
// INPUTS: { status: &syn_core::hooks::GitHookStatus }
// OUTPUTS: { anyhow::Result<()> }
// SIDE_EFFECTS: writes stdout
// LINKS:
...
fn print_git_hook_status(status: &syn_core::hooks::GitHookStatus) -> anyhow::Result<()> {
    match status {
        syn_core::hooks::GitHookStatus::Active { path } => {
            println!("pre-commit hook status: active at {path}");
        }
        syn_core::hooks::GitHookStatus::NotInstalled { path } => {
            println!("pre-commit hook status: not-installed at {path}");
        }
        syn_core::hooks::GitHookStatus::Foreign { path, message } => {
