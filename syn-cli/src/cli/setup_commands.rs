...
use syn_engine::grace::bootstrap::bootstrap_existing_repo;
...
impl InitCmd {
    ...
    pub async fn run(&self, config: Config) -> anyhow::Result<()> {
        ...
        let merged_config = syn_core::hooks::merge_synapse_mcp_config(&existing_config)?;
        ...
        let walker = syn_engine::indexer::walker::Walker::new(&root);
        ...
        if matches!(
            syn_core::hooks::HookManager::new(&config).git_pre_commit_status(),
            Ok(syn_core::hooks::GitHookStatus::NotInstalled { .. })
        ) {
...
fn classify_watch_event(event: &notify::Event) -> (Vec<PathBuf>, Vec<PathBuf>) {
    ...
    for path in event
        .paths
        .iter()
        .filter(|path| syn_engine::indexer::walker::detect_language(path).is_some())
    {
        if is_delete {
