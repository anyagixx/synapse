...
impl SearchCmd {
    ...
    pub async fn run(&self, config: Config) -> anyhow::Result<()> {
        ...
        for (i, r) in results.iter().enumerate() {
            ...
            let preview = syn_core::utils::truncate_chars(&normalized, 120);
            println!(
                "{}. {}:{} ({} {}) score={:.1} [{}]\n   {}",
                i + 1,
                r.path,
...
impl GrepCmd {
    ...
    pub async fn run(&self, _config: Config) -> anyhow::Result<()> {
        use syn_engine::indexer::parser::ParserEngine;
        ...
        for file in &files {
            ...
            for block in &blocks {
                ...
                if kind_match || name_match || content_match {
                    ...
                    let preview = syn_core::utils::truncate_chars(&normalized, 200);
                    println!(
                        "{}:{} — {} ({})",
